...
parser.add_argument('--max_lr', type=float, default=0.001, help='Maximal LR for CLR.')
...
def main():
    ...
    train_loader, val_loader = get_loaders(args.dataroot, args.batch_size, args.batch_size, 160,
                                           args.workers)
...
def train_network(start_epoch, epochs, scheduler, model, train_loader, val_loader, optimizer, criterion, device, dtype,
    ...
    for epoch in trange(start_epoch, epochs + 1):
        ...
        test_loss, test_accuracy1, test_accuracy5 = test(save_path, model, val_loader, criterion, device, dtype)
