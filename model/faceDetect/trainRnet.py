...
from faceDetectPnet import RNet, LossFn
...
def main():
	...
	save_path = os.path.join(args.results_dir, time_stamp + 'RNet')
	...
	model = RNet(is_train=True, use_cuda=True)
	if args.gpus is not None:
	...
	imagedb_valid = ImageDB(os.path.join(config.ANNO_STORE_DIR,
	                                     config.RNET_TRAIN_IMGLIST_FILENAME_VALID), 'train')
