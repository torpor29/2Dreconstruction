...
from faceDetectPnet import ONet
import utils
from utils import convert_chwtensor_to_hwcnumpy
from utils import convert_image_to_tensor

def creat_pronet(p_model_path = None, r_model_path = None, o_model_path = None, device = None):
	...
	if o_model_path is not None:
		onet = ONet(is_train=True, use_cuda=True)
		onet = torch.nn.DataParallel(onet, [1])
		onet.to(device=device, dtype=torch.float32)
		print("=> loading checkpoint '{}'".format(o_model_path))
		checkpoint = torch.load(o_model_path, map_location=device)
		onet.load_state_dict(checkpoint['state_dict'])
	onet.eval()

	return pnet, rnet, onet

class PROnetDetector(object):
	def __init__(self, pnet=None, rnet=None, onet=None, min_face_size=12, stride=2, threshold=[0.6, 0.7, 0.7], scale_factor=0.709):
	...
	def detect_face(self, img):
		...
		if self.onet_detector:
			boxes, boxes_align = self.detect_onet(img, boxes_align)
			if boxes_align is None:
				return np.array([]), np.array([])
