...
from faceDetectPnet import PNet, LossFn
import PNetdataset
...
def main():
	...
	model = PNet(is_train=True, use_cuda=True)
	if args.gpus is not None:
	...
	imagedb_valid = ImageDB(os.path.join(config.ANNO_STORE_DIR,
	                                     config.PNET_TRAIN_IMGLIST_FILENAME_VALID), 'train')
...
def test(save_path, model, loader, lossfn, device, dtype):
	...
	for batch_idx, (path, data, cls, bbox) in enumerate(tqdm(loader)):
		...
		with torch.no_grad():
			...
			outputbbox = outputbbox.cpu().numpy()
			for i in range(0, len(path)):
				newline = path[i]
				for j in range(0, 4):
					newline = newline + ' ' + str(outputbbox[i][j])
				f.write(newline+'\n')
	f.close()
