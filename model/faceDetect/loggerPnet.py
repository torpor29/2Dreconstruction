...
class CsvLogger:
    def __init__(self, filepath='./', filename='results.csv', data=None):
        ...
        self.fieldsnames = ['epoch', 'accuracy', 'cls_loss', 'bbox_loss', 'test_loss']
    ...
    def plot_progress_errk(self, claimed_acc=None, title='Pnet', k=1):
    ...
    def plot_progress_loss(self, title='Pnet'):
        plt.figure(figsize=(9, 8), dpi=300)
        plt.plot(self.data['accuracy'], label='accuracy')
        plt.plot(self.data['cls_loss'], label='cls_loss')
        plt.plot(self.data['bbox_loss'], label='bbox_loss')
        plt.plot(self.data['test_loss'], label='test_loss')
        plt.title(title)
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        plt.xlim(0, len(self.data['accuracy']) + 1)
        plt.savefig(os.path.join(self.log_path, 'loss.png'))

    def plot_progress(self, title='RNet'):
